...
I start to love using docker for setting up development environment

This month I decided to pick docker as technology that I will be studying. At first it was quite hard to grasp some of the concepts. But eventually I start to get it. As for referrence I stop the udemy course that I purchase since it was too much theoritical and not enough practical applications.

Since I have some basic expirience with it. I’ve decided to move my personal blog site to dockerize container in my hosting. Before I was using Linode but I’ve decide to move to Digital Ocean since it was much more modern and there are a lot of options in my opinion.  I backup my wordpress sites and the database and I delete my monthy vps.

Interesting that I could put a lot of stuff on my current $5/ month in my digital ocean account when I start using docker.

On my wordpress site it was using 7.0 version of php but the current image that I’ve setup is using 7.3 so there are some brocken parts of my site so it was breese to just change the docker configurations and docker will do the necessary changes for you. In normal development environment this will be hard to implement since it might affect the other applications that you have.

I know that the are some simple tools such as Heroku but I prefer to setup on my own since I have some custom setups that I will be doing soon such as I will use ghost platform for my technical blog that I will write soon together with my wordpress on it’s side.

Plus I’ll be using my current setup to host some of clients projects that doesn’t have yet hosting but needs a beta link in order to view the updates on their site. Ohh I’ll be putting here some of my hobby project that I’ll be making.

I will write more about my expirience in docker for my technical blog once I set it up. 
